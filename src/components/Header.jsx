      ...
      <div className={`flex items-center justify-center w-full lg:px-7.5 xl:px-10 max-lg:py-4`}>
        ...
        <nav className={`${openNavigation ? "flex bg-white " : "hidden"} fixed items-center justify-center top-[5rem] left-0 right-0 bottom-0 lg:static lg:flex lg:mx-auto lg:bg-transparent`}>
          <div className='relative z-10 flex flex-col items-center justify-between m-auto lg:flex-row'>
            ...
            {navigation.map((item) => (
              <a key={item.id} href={item.url} onClick={handleClick}  className={`block relative font-code  uppercase text-p-8 transition-colors hover:text-p-10 px-6 py-6 md:py-8 lg:-mr-0.25 lg:text-xs sm:text-xs lg:font-semibold  ${
                item.onlyMobile ? "lg:hidden" : ""}  ${item.url === pathname.hash? "z-20 lg:text-p-10" : "lg:text-p-10/80"} lg:leading-5 lg:hover:text-p-10 xl:px-12`}>
                {item.title}
              </a>
            ))}
          </div>
        ...
        <Button className="btn-secondary  hidden lg:flex lg:text-xs px-6 py-3 uppercase text-p-8 sm:text-xs lg:font-semibold" href="/login">
          Sign In
        </Button>
